...
void summerSqlAnalyse(void *sql, unsigned char *ptype, void *parameters)
{ 
...
//	summerSqlExecute(type, sql + 1);
 	switch(type){
		...
		case CTB:						//create table
				int16_t attr_num = *(int16_t *)(sql + 1);
				int16_t size = 2 + 4 + 8 + 2 + attr_num * 2 + 64 + 72 * column_num;
				// joint a tuple
				parameter =(void *)malloc(size * sizeof(char));
				memset(parameter, 0, size * sizeof(char));
				*(int16_t *)(paramter) = size -2;
				//tuple headinfo
				*(page_no *)parameter = 0;
				...
				*(int16_t *)(parameter + 14) = attr_num;
				...
				//attribute
				for(i = 0; i < attr_num; i++){
					attributeType *attr = (attributeType *)(*(int *)(sql + 7 + 4 * i));
					//attribute name
					...
					//attribute's attr
					memcpy(schemeInfo + 67, &attr->length, 1);
					schemeInfo += 68;
				} 
				break;
...
void summerSqlExecute(unsigned char type, void *parameters)
{
	switch(type){
		case CDB:			//create databae
				char *dbname = (char *)(*(int *)parameter);
				createDatabase(dbname);
				break;
		case CTB:			//create table
				tuple *new_tuple = (tuple *)malloc(sizeof(tuple));
				new_tuple->ptr = parameters + 2;
				new_tuple->size = *(int16_t *)parameter;
				int16_t attr_num = *(int16_t *)(parameter + 14);
				char *tname = (char *)(parameter + 16 + attr_num * 2);
